...
export function apply(ctx, config) {
    ...
    ctx.inject(['settings'], (settingsCtx) => {
        ...
        const applySettings = (value) => {
            if (value?.wallpaperDir)
                state.wallpaperDir = value.wallpaperDir;
            else
                state.wallpaperDir = config?.wallpaperDir ?? DEFAULT_WALLPAPER_DIR;
            if (value?.weAssetsDir)
                state.weAssetsDir = value.weAssetsDir;
            else
                state.weAssetsDir = config?.weAssetsDir ?? DEFAULT_WE_ASSETS_DIR;
        };
