...
export function buildInitialObjectState(origin, scale, alpha, brightness) {
    return {
...
export function normalizeReadback(raw) {
    const rb = {};
    if (raw.origin)
        rb.origin = { x: raw.origin.x, y: raw.origin.y, z: raw.origin.z };
    if (raw.scale)
        rb.scale = { x: raw.scale.x, y: raw.scale.y, z: raw.scale.z };
    if (raw.imageAlpha !== undefined)
        rb.imageAlpha = Math.max(0, Math.min(1, raw.imageAlpha));
    if (raw.imageBrightness !== undefined)
        rb.imageBrightness = raw.imageBrightness;
    return rb;
}
