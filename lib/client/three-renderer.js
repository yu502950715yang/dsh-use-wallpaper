...
import { resolveWorldTransforms } from './scene-graph.js';
...
import { resolveVisibility, isStaticallyHidden } from './visibility.js';
...
let lastWorldTransforms = null;
export function lastWorldTransformOf(id) {
...
// 静态隐藏判定（可选）：隐藏对象不参与渲染 ⇒ 也不解析它的链（省掉整条链的 fetch 与 runner）。
// 缺省不过滤 → 既有调用/单测行为逐字不变。
isHidden) {
    const out = new Map();
    for (const group of groupEffectsByObject(desc.objects)) {
        if (isHidden?.(group.obj))
            continue;
        const chains = [];
...
export function collectScriptSources(desc) {
    const out = [];
    for (const obj of desc.objects) {
        ...
        if (obj.visible?.kind !== 'script')
            continue;
        const s = obj.script;
        if (typeof s === 'string' && s.length > 0)
            out.push({ objectId: obj.id, source: s });
    }
    return out;
}
...
export function createThreeSceneRenderer(opts) {
    ...
    // ── 壁纸音效（A3）──────────────────────────────────────────────────────────────
    // sound 对象 → playWallpaperSound（loop）→ analyser；频谱每帧喂给效果链（Simple_Audio_Bars 等）。
    let currentAudio = null;
    // 运行期音效开关（面板 soundEnabled）：null = 未下发 ⇒ 用装配期读到的设置值。
    let soundEnabledOverride = null;
    // 最近一次装配的音频规格：运行期重新打开时按它重播（不必重选壁纸）。
    let lastAudioSpec = null;
    const stopAudio = () => {
        const audio = currentAudio;
        currentAudio = null;
        // 必须 close：每个壁纸一个 AudioContext，切壁纸不释放会一路累积到浏览器上限。
        if (audio)
            void audio.analyzer.context.close().catch(() => { });
    };
    const startAudio = (id, sounds, enabled) => {
        ...
        if (!analyzer)
            return; // 无 Web Audio → 静音回退（不阻断渲染）
        currentAudio = { analyzer, id, sounds };
        // 失败的 sound（404/解码失败）在 playWallpaperSound 内部静默降级，不影响其余与画面。
    ...
    const teardown = () => {
        ...
        for (const b of currentScriptBindings)
            b.dispose();
        currentScriptBindings = [];
        // SceneScript 运行时（ctx + 全部句柄）同理由本次装配独占。
        if (scriptClickTarget && scriptClick)
            scriptClickTarget.removeEventListener('click', scriptClick);
        ...
        // 壁纸音效随本次装配一起释放（AudioContext 不 close 会一路累积到浏览器上限）。
        stopAudio();
    };
    ...
    return {
        async render(id, fg, _bg) {
            ...
            try {
                ...
                // 用户属性表（visible 的 {user,key} 绑定）：image/particle/text 三处过滤与 SceneScript
                // 共用，故在对象循环之前建好。
                const userProps = {};
                for (const obj of desc.objects) {
                ...
                for (const obj of desc.objects) {
                    // 装载期静态隐藏（2026-09-25）：布尔 / 用户属性绑定在此定论；script 绑定不在此列
                    // （其可见性是 visible.script 的每帧返回值，装载期剔掉脚本就再也打不开它）。
                    if (isStaticallyHidden(obj.visible, userProps))
                        continue;
                    if (obj.kind === 'image') {
                ...
                // ── text 对象（2026-09-21）────────────────────────────────────────────────
                // text 用 resolveVisibility（**含** script 绑定按 value 静态定论）：本库 text 全是脚本时钟，
                // 其中数个默认隐藏（2911105183 的 3 个 Clock 有 2 个 value=false），不过滤就会叠出多个时钟。
                // ⚠️ 与 image/particle 的 isStaticallyHidden 有意不同（后者放过 script 绑定）：text 的历史
                // 口径保持原样，不借本次改动改它的行为。
                ...
                for (const obj of desc.objects) {
                    ...
                    if (binding) {
                        ...
                        textLayers.set(obj.id, {
                            ...
                            driver: createScriptDriver(scriptTexture.image, opts, binding, initial),
                            size: [size.w, size.h],
                            anchorOffset,
                        });
                        continue;
                ...
                const effectChains = await collectObjectEffectChains(desc, loadFile, (o) => isStaticallyHidden(o.visible, userProps));
                ...
                // 壁纸音效（A3）：sound 对象默认播放（对齐桌面 WE，可用面板关掉）。
                // 关掉时**不创建** AudioContext —— 否则 25/29 张无音效的壁纸也白开一个音频上下文。
                ...
                for (const obj of desc.objects) {
                    // 静态隐藏的对象不隔离：隔离会给它建对象 RT + 合成 quad ⇒ 本应隐藏的层又画回来。
                    if (isStaticallyHidden(obj.visible, userProps))
                        continue;
                ...
                const result = loadSceneToThree(sceneJson, {
                    ...
                    onFrame: (dt) => {
                        // 音频频谱（A3）：本帧渲染前刷新一次；advance 时 runner 读同一缓冲（无音频 → null）。
                ...
                if (scriptSources.length > 0) {
                    ...
                    for (const p of materialPaths) {
                        ...
                        try {
                            ...
                            if (spec.texturePath) {
                                ...
                                if (!tex)
                                    warnOnce(`mesh-tex:${id}:${texPath}`, `动态网格纹理加载失败：${texPath}（白图兜底）`);
                            }
                    ...
                    if (currentScriptHost) {
                        ...
                        console.log(`[three] scene scripts id=${id} collected=${scriptSources.length} loaded=${currentScriptHost.scriptCount} active=${currentScriptHost.activeCount} meshes=${meshRegistry.modelCount} materials=${materialTable.size}`);
                    }
                }
        ...
        setSoundEnabled(value) {
            ...
            if (!currentAudio && lastAudioSpec)
                startAudio(lastAudioSpec.id, lastAudioSpec.sounds, true);
        },
