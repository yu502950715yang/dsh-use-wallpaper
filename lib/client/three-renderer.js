...
export function createThreeSceneRenderer(opts) {
    ...
    // 本次装配的**资源纹理**（背景 image 对象）：teardown 时显式 dispose。
    // 为什么必须有这一步（2026-09-15）：`player.dispose()` 里的 `renderer.dispose()` 只释放 WebGL
    // 资源，**不会停解码** —— 视频纹理（WE flags bit5）背后是 `<video>` + Blob URL（实测单张 mp4
    // 10 MB、27s），其清理挂在纹理的 `dispose` 事件上（见 tex-loader 的 videoTextureFromMp4）。
    // 不释放 ⇒ 每切一次壁纸就多一个仍在解码的视频与一份 Blob，内存/CPU 双泄漏。
    ...
    const teardown = () => {
        ...
        for (const sim of current?.sims ?? [])
            sim.free?.();
        current = null;
        // 纹理放在播放器之后释放：GPU 侧先随 renderer.dispose 收口，再触发纹理自身的清理钩子
        // （视频纹理在这里 pause/清 src/revokeObjectURL）。
        ...
        async render(id, fg, _bg) {
            ...
            try {
                ...
                for (const [objId, chains] of effectChains) {
                    ...
                    if (isolate.has(objId) || rtGraphOnly.has(objId))
                        continue;
