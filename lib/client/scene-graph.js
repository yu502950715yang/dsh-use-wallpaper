...
const MAX_DEPTH = 64;
const LOCAL_ZERO = [0, 0, 0];
const LOCAL_ONE = [1, 1, 1];
...
function rotationMatrix(angles) {
    return new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(angles[0], angles[1], angles[2], ANGLE_ORDER));
}
export function resolveWorldTransforms(nodes) {
    const byId = new Map();
    ...
    const resolve = (n, depth) => {
        ...
        const offset = new THREE.Vector3(self.origin[0] * pw.scale[0], self.origin[1] * pw.scale[1], self.origin[2] * pw.scale[2]).applyMatrix4(rotationMatrix(pw.angles));
