...
export async function resolveParticleMaterial(id, specText) {
    try {
        ...
        if (typeof texName !== 'string' || !texName)
            return { texUrl: null, blending };
        // host 路由 /wallpapers/particle-texture 以 `<weAssetsDir>/assets/materials` 为基准，
        // 而 WE 内置粒子纹理都在 `particle/` 下 → name 统一拼 `particle/` + （去前缀后的）路径。
        // 例： "particle/fog/fog1" → name=particle/fog/fog1；"presets/lightshaft" → 别名
        // "light/light_shafts_0" → name=particle/light/light_shafts_0。
        ...
        return {
            texUrl: `/wallpapers/particle-texture?name=${encodeURIComponent(`particle/${resolved}`)}`,
            blending,
        };
    }
    catch {
...
export async function resolveParticleTexUrl(id, specText) {
                ...
                for (let i = 0; i < desc.objects.length; i++) {
                    ...
                    if (obj.kind === 'image') {
                        ...
                        if (obj.script && obj.script.trim()) {
                            ...
                            if (scriptRuntime)
                                currentScriptRuntime = scriptRuntime; // 持引用，teardown 时 dispose
                        ...
                        if (isObjectPath) {
                            ...
                            if (chainDesc.length > 0) {
                                ...
                                scene.set_particle_sim(specText, Float32Array.from(obj.origin), texBytes ?? new Uint8Array(0));
                            }
                        }
                        else {
                            scene.set_particle_sim(specText, Float32Array.from(obj.origin), texBytes ?? new Uint8Array(0));
