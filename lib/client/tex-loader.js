...
function pickMipmap(mips) {
    let best = null;
    for (const m of mips) {
        if (m.width <= 2048 && (!best || m.width > best.width))
            best = m;
    }
...
export async function textureFromTex(info) {
    ...
    if (mime) {
        ...
        try {
            ...
            const bitmap = await createImageBitmap(new Blob([mip.data], { type: mime }), { imageOrientation: 'flipY' });
    ...
    if (glFormat) {
        const tex = new THREE.CompressedTexture(info.mipmaps.map((m) => ({ data: m.data, width: m.width, height: m.height })), mip.width, mip.height, glFormat);
        tex.needsUpdate = true;
        return tex;
    }
    return null;
...
export async function loadTexTexture(url) {
