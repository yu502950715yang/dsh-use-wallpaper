...
import { CAMERA_DISTANCE, OBJECT_RT_MAX, PARTICLE_DEFAULT_DISTANCE, materialModulation, objectCameraRange, particleObjectRange, particleWorldSize, createObjectRenderTarget, shouldUseObjectPath, groupEffectsByObject, PendingChainStore, uvWindow, createCompositeGeometry, coverRange, containRange, } from './object-range.js';
...
export function barAnchorOffsetY(alignment, height) {
    if (alignment === 'bottom')
        return height / 2;
    if (alignment === 'top')
        return -height / 2;
...
export function updateVisualizerBars(bars, anchorY, // 三坐标系锚点 y（对象 origin 的中心映射，不翻转）
props, // 已解包的 scriptProperties
freqData) {
...
export function createSceneRenderer(fgCanvas, bgCanvas, audioAnalyzer) {
    ...
    const screenQuad = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), new THREE.MeshBasicMaterial({ map: sceneRT.texture, transparent: true }));
    ...
    function createObjectEntry(objId, origin, worldW, worldH, range, content) {
        ...
        const localCamera = new THREE.OrthographicCamera(-rt.width / 2, rt.width / 2, rt.height / 2, -rt.height / 2, -1000, 1000);
        ...
        const quad = new THREE.Mesh(createCompositeGeometry(worldW, worldH, rt.width, rt.height), new THREE.MeshBasicMaterial({ map: rt.texture, transparent: true }));
        ...
        if (pending)
            applyObjectChains(objId, pending.chains, pending.wallpaperId);
    }
    function frame() {
        ...
        for (const v of visualizerEntries)
            updateVisualizerBars(v.bars, v.anchorY, v.props, freqData);
        for (const c of clockDrivers)
            c.update(new Date());
        ...
        if (running && fgCanvas.isConnected)
            raf = requestAnimationFrame(frame);
        else
            stop(); // canvas 被 controller 移除（切换壁纸）时自动终止 raf，防止泄漏
    }
    return {
        setScene(desc) {
            scene.clear();
        ...
        setImageObject(tex, obj) {
            ...
            if (shouldUseObjectPath(obj)) {
                createObjectEntry(obj.id, center, worldW, worldH, objectCameraRange([w, h], [s[0], s[1]]), mesh);
                return;
            }
        ...
        setTextObject(tex, obj) {
            ...
            const s = obj.scale;
            mesh.scale.set(s[0], s[1], s[2] ?? 1);
            mesh.position.set(obj.origin[0] + off[0] - ortho.width / 2, obj.origin[1] + off[1] - ortho.height / 2, obj.origin[2]);
            scene.add(mesh);
        },
        ...
        setObjectEffectChains(objId, chains, wallpaperId) {
            ...
            if (!pendingChains.applyIfReady(objId, { chains, wallpaperId }, objectEntries.has(objId)))
                return;
...
export function resolveTexPath(matRef, texName) {
...
export async function resolveImageTexture(id, obj) {
    try {
...
export async function renderScene(id, fgCanvas, bgCanvas, opts) {
    let renderer = null;
    ...
    try {
        ...
        for (const obj of desc.objects) {
            const v = obj.visible;
            if (v?.kind === 'user' && v.key)
                userProps[v.key] = getUserProperty(v.key);
        }
        ...
        for (const obj of visibleObjects) {
            if (obj.kind === 'image') {
                ...
                if (!tex)
                    continue; // 纹理缺失 → 跳过该对象（骨架注记：失败即跳过）
