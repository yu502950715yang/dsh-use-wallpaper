...
export function extractUniformAnnotations(source) {
    const out = [];
...
export function extractIfIdentifiers(src) {
    const out = new Set();
...
// 导出供 glsl-to-naga（wasm 路径）复用（Task B；导出不改变既有行为）。
export function extractComboDefaults(src) {
    const out = new Map();
...
const VARYING_COMPONENTS = { float: 1, vec2: 2, vec3: 3, vec4: 4, mat2: 4, mat3: 9, mat4: 16 };
const VEC_VARYING_TYPES = new Set(['vec2', 'vec3', 'vec4']);
/** 抓 `varying <type> <name>;` 声明映射（同名多次声明时后者覆盖）。 */
function extractVaryingDecls(src) {
    const out = new Map();
    for (const m of src.matchAll(VARYING_DECL_RE))
        out.set(m[2], m[1]);
    return out;
}
/** name 的每处出现（剔除其声明行后）是否都紧跟 `.`（纯分量/swizzle 访问）。 */
function usesOnlySwizzle(src, name) {
    const body = src.replace(new RegExp(`^[ \\t]*varying[ \\t]+(?:vec[234]|float|mat[234])[ \\t]+${name}[ \\t]*;`, 'gm'), '');
...
export function reconcileVaryingDeclarations(rawVert, rawFrag) {
    ...
    for (const [name, fragType] of fragDecls) {
        ...
        if (vertType === undefined || vertType === fragType)
            continue; // 只处理两侧都声明且类型不同
        ...
        frag = frag.replace(new RegExp(`^([ \\t]*)varying[ \\t]+${fragType}[ \\t]+${name}([ \\t]*);`, 'gm'), `$1varying ${vertType} ${name}$2;`);
    }
...
export function normalizeFloatIntLiterals(src) {
...
export function floatifyIntVarUses(src) {
...
export function relaxGlsl3Strictness(src) {
    let out = src
        // const 非常量初始化 → 去 const（局部与全局都处理；纯字面量保持 const）
        .replace(/\bconst\s+(float|int|vec[234]|mat[234])\s+(\w+)\s*=\s*([^;]*[A-Za-z_][^;]*);/g, (m, type, name, expr) => {
        const trimmed = expr.trim();
        if (/^-?[\d.]+$/.test(trimmed) || /^(true|false)$/.test(trimmed))
            return m;
    ...
    if (mainIdx > 0) {
        ...
        for (let i = 0; i < mainIdx; i++) {
            ...
            if (!m)
                continue;
            const expr = m[3].trim();
            if (/^-?[\d.]+$/.test(expr) || /^(true|false)$/.test(expr))
                continue; // 纯常量
            if (/^(?:CAST[234]|vec[234]|mat[234])\s*\(\s*-?[\d.]+\s*\)$/.test(expr))
                continue; // 纯常量构造
...
export function markHeaderText(text) {
    return `${HEADER_BEGIN}${text}${HEADER_END}`;
}
/** 剥掉区段标记（最终源码不需要它们）。 */
export function stripHeaderMarks(text) {
...
function collectIntVarNames(text) {
    const out = new Set();
    for (const m of text.matchAll(/\b(?:const\s+)?(?:uniform\s+)?(?:in\s+|out\s+)?int\s+(\w+)(?!\s*\()/g))
        out.add(m[1]);
    return out;
}
...
function collectIntParamPositions(src) {
    ...
    for (const m of src.matchAll(/\b(?:void|float|int|uint|bool|vec[234]|mat[234])\s+(\w+)\s*\(([^)]*)\)\s*\{/g)) {
        const pos = new Set();
        m[2].split(',').forEach((p, i) => { if (/\bint\b/.test(p))
            pos.add(i); });
        if (pos.size)
            out.set(m[1], pos);
    }
...
function protectIntParamArgs(text, signatures, protect) {
    ...
    while ((c = callRe.exec(text))) {
        // 跳过函数定义本身（其前缀是返回类型）
        if (/\b(?:void|float|int|uint|bool|vec[234]|mat[234])\s+$/.test(text.slice(0, c.index)))
            continue;
...
export function protectIntContexts(src) {
    const blocks = [];
    const protect = (m) => {
    ...
    for (const name of collectIntVarNames(body)) {
        ...
        while ((m = re.exec(text))) {
            ...
            if (/\b(?:const|uniform|in|out|int|uint|float|bool|vec[234]|mat[234])\s+$/.test(before))
                continue;
            spans.push([m.index, m.index + m[0].length]);
        }
        for (const [s, e] of spans.reverse())
            text = text.slice(0, s) + protect(text.slice(s, e)) + text.slice(e);
    }
    //  - int 形参位置上的实参
...
export function applyGlsl3StrictnessFixes(src) {
...
function undefHeaderMacroRedefinitions(src) {
    return src.replace(/^([ \t]*)#define[ \t]+([A-Za-z_][A-Za-z0-9_]*)([^\n]*)$/gm, (whole, indent, name, rest) => 
    // rest 含行尾 \r，\n 单独插入 ⇒ 不破坏 CRLF 行的宏体
    HEADER_MACRO_NAMES.has(name) ? `${indent}#undef ${name}\n${indent}#define ${name}${rest}` : whole);
}
export function preprocessWeShader(source, combos) {
    ...
    for (const m of out.matchAll(/^\s*#define\s+([A-Za-z_][A-Za-z0-9_]*)/gm))
