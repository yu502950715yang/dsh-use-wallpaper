...
export class ThreeScenePlayer {
    ...
    writeParticleData(layer, data, count) {
        ...
        const minCapacity = Math.min(Math.floor(layer.positions.array.length / 3), layer.sizes.array.length, Math.floor(layer.uvs.array.length / 2), Math.floor(layer.colors.array.length / 3), layer.alphas.array.length, layer.rots.array.length);
