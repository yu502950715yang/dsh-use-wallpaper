...
export function resolvePixelRatio(devicePixelRatio, qualityScale) {
...
export class ThreeScenePlayer {
    ...
    screenScalePx() {
        return screenScalePx(this.sceneWidth, this.sceneHeight, this.viewWidth, this.viewHeight, this.pixelRatio);
    }
    ...
    render() {
        ...
        if (this.glowStage)
            this.glowStage.apply(this.renderer, this.scene, this.camera);
        else
    ...
    addBackground(opts) {
        ...
        if (opts.objectId !== undefined) {
            ...
            if (shown)
                this.displayObjects.set(opts.objectId, shown);
        }
        return id;
    }
    ...
    attachIsolated(objectId, kind, content, worldW, worldH, size, position, quadAngles, colorBlendMode) {
        ...
        const quad = new THREE.Mesh(compositeGeometry, this.createCompositeQuadMaterial(rt.texture, colorBlendMode));
    ...
    resizeBackground(id, size) {
        ...
        if (!entry)
            return;
        const [w, h] = size;
        if (w === entry.size[0] && h === entry.size[1])
            return;
        entry.size = [w, h];
    ...
    writeParticleData(layer, data, count) {
        ...
        const minCapacity = Math.min(Math.floor(layer.positions.array.length / 3), layer.sizes.array.length, Math.floor(layer.uvs.array.length / 2), Math.floor(layer.colors.array.length / 3), layer.alphas.array.length, Math.floor(layer.rots.array.length / 3));
...
export function loadSceneToThree(sceneJson, assets, canvas, viewport) {
    ...
    const world = (o) => {
        ...
        return w
            ...
            : { origin: o.origin, scale: o.scale, angles: o.angles ?? [0, 0, 0] };
    };
