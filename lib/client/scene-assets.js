...
export async function fetchSceneDescription(id) {
    const resp = await fetch(`/wallpapers/scene/${id}/asset?name=scene.json`);
    if (!resp.ok)
        throw new Error('scene.json fetch failed');
    return parseSceneJson(await resp.text());
}
export function particlesFromSpec(root) {
...
export async function fetchParticleSpec(id, assetName) {
