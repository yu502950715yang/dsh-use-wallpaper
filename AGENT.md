...
6. **text 对象已渲染（2026-09-21），但脚本只覆盖「时钟」一类**：three 主路径把 text 当背景 quad（与 image 同路径）渲染，并**只对 text 应用 `visible` 过滤** —— 不接会叠出本应隐藏的时钟（`2911105183` 的 3 个 Clock 里 2 个 `value=false`，靠用户属性切换）。脚本侧只实现 `detectScriptPattern === 'clock'` 那一类：**端到端已验证** `2937346640` 画面上出现 `PM 02:16 Sep. 20 2026`（harness 冻结时钟，`console error = 0`）；**带脚本但未识别的 text 一律跳过（不渲染）**：脚本不执行时 `text.value` 只是作者的占位值，画出来是屏幕脏字 —— 2026-09-21 用户实测 `2980088441`（CodeTime）屏幕上只剩 `"12"` / `hour:minute:frame:…` / `"Year"`；改为跳过后该壁纸零内容、走 preview 回退（e2e `harness ok=false`，console error = 0），`3765967112` / `3789452668` 也不再凭空多出 `DAY` / `<Date>` 占位行。本机 28 张库里 20 个 text 对象只有 1 个（`2937346640`）落在 clock 模式，其余 19 个是「每对象一段脚本」等未识别写法。执行任意壁纸脚本（`quickjs-emscripten` 已在依赖里，`scene-script.ts` 有先例）是后续候选，**未做**。`image` / `particle` 的 `visible` 仍未过滤（全库 22 个非平凡绑定里 16 个在这两类上）。**text 的 effects 未隔离**（见第 1 条末尾的订正）。字体：pkg 内 `.otf/.ttf` 经 `FontFace` 加载（`fonts/workshop/<id>/xxx.otf` 那类），失败或环境不支持（jsdom）回退 sans-serif。
