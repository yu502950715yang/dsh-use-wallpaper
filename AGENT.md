...
4. **`copybackground: true` 的图层走隔离路径时接近全黑（2026-09-14 发现，既有缺陷，未修）**：`2597392171`（全库 RT 显存最重的那张，scene 固有尺寸 3840×6000）的两个背景对象 obj 45/50 都是**铺满全场**的 `copybackground: true` 图层（scroll + waterripple + shake），隔离路径成品的平均亮度 **15.3/255**，而同一壁纸去掉 effects（不隔离、主相机直渲）是 **81.0/255**；逐像素 MAD **89.0**、99.8% 像素差异 ≥2。**A/B 证明与本次「清晰度口径」修复无关**：旧口径同样暗（平均亮度 14.98、MAD 88.2），新口径只是稍亮（15.3）—— 「隔离后变暗」是既有行为（新口径下 Laplacian 2.0 → 10.4 不是锐度提升，而是「原本几乎纯黑的画面」多了一点结构；直渲 142.4）。**疑因**（未确证，待专项）：WE 的 `copybackground` 语义是「该图层的 RT 先拷入其下方已绘制的背景、再在 RT 上跑效果链」（水面倒影/涟漪这类效果读的就是背景副本），我们隔离时 RT 一律**清成透明黑**再画对象内容 ⇒ 内容本身近乎空 ⇒ 合成后整屏发黑。旁证：这两个对象的 effects 里还有 `"visible": false` 与绑定场景脚本属性的 **effect 级可见性**，我们只解析对象级 `visible`、不解析 effect 级 ⇒ 本不该跑的 pass 也会生效。
