...
- 注释**从简**（2026-09-15 用户要求）：一行说清「是什么 / 为什么」，最多 2 行；不复述代码、不写长推理与实验数据。根因与踩坑史写进本文件（§5 / §7）或 `docs/`，代码里只留一句结论 + `AGENT.md §x.y` 指针。
