...
// 缺省 Wallpaper Engine 安装目录（2026-08-21 方案 A：wasm 粒子纹理——粒子材质
// textures 如 "particle/fog/fog1" 是引擎内置资源，从安装目录 assets/materials/ 读取；
// cordis.patch.yml 的 config.weAssetsDir 可覆盖）
...
export function apply(ctx: any, config?: WallpaperEngineConfig): void {
  ...
  const weAssetsDir = config?.weAssetsDir ?? DEFAULT_WE_ASSETS_DIR;
  ...
  // /wallpapers/static、/wallpapers/web、/wallpapers/particle-texture）
  registerWallpaperRoutes(ctx, { wallpaperDir, staticDir: DEFAULT_STATIC_DIR, weAssetsDir });
}
