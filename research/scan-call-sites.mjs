// 提取 3 参 texSample2D 与 ApplyComposite 的完整调用行（研究用）
...
for (const id of readdirSync(WALLPAPER_DIR)) {
  ...
  for (const [name, data] of files) {
    ...
    for (const line of src.split('\n')) {
      const t = line.trim();
      if (/texSample2D\s*\([^)]*,[^)]*,[^)]*\)/.test(t)) console.log(`[${id}] ${name}: ${t.slice(0, 160)}`);
      if (/ApplyComposite(?:Offset)?\s*\(/.test(t) && !t.includes('vec4 ApplyComposite')) console.log(`[${id}] ${name}: ${t.slice(0, 160)}`);
    }
  }
}
