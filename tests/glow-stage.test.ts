...
describe('normalizeGlowOptions（参数 clamp：越界 / 非法一律收敛，不抛）', () => {
  it('缺省 = 2026-09-21 用户真机定档 (0.65 / 0.35)', () => {
    expect(normalizeGlowOptions()).toEqual({ threshold: 0.65, strength: 0.35 });
    expect(GLOW_DEFAULTS).toEqual({ threshold: 0.65, strength: 0.35 });
  });
  ...
  it('NaN / 非数字回退缺省（不把 NaN 灌进 uniform）', () => {
    expect(normalizeGlowOptions({ threshold: NaN, strength: NaN })).toEqual({ threshold: 0.65, strength: 0.35 });
