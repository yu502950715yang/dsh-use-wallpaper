...
- **文本对象的脚本只覆盖「时钟」一类写法**：时间/日期会实时走字（VHS 时间与日期那类已验证）；其它写法的脚本文本（例如把时/分/月/日拆成多个文本层、各写一段脚本的 2980088441）**不会显示** —— 脚本不在执行范围，直接画作者的占位值（`12` / `mounth` / `DAY`）会变成屏幕上的脏字，所以选择跳过，这类壁纸的文本层保持空缺。
