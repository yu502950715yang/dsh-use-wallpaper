...
- **粒子的自转已实现**（2026-09-22）：花瓣、雪花的翻滚/倾斜现在按壁纸自身的 `rotationrandom` / `angularvelocityrandom` 渲染。**但能不能看出来取决于壁纸**：只有粒子规格里给了**非零**的旋转/角速度范围才会动 —— 本机 29 张里实测 **12 张**（19/121 个粒子对象）满足，其余多为该字段没有 `min`/`max` 或全零。
