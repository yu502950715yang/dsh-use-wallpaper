...
- **应用级光晕（Glow）已实现并默认开启**（设置 → 壁纸 → 「光晕」可关）：scene 壁纸的亮部（路灯、霓虹、云边）现在有与桌面版基本一致的光晕（实测云区亮度 p99：关 200 → 开 224，桌面 225），此前"桌面更亮"的主要差异来源即它。**仅 scene 壁纸**：视频 / 图片 / 网页壁纸的亮部仍与桌面有差。**目前只在一张壁纸（GTR）上真机验证过**。
