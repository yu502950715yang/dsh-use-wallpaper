...
> 已发布到 npm，最新版本 **[0.4.0](https://www.npmjs.com/package/@dsh-use/wallpaper-engine)**。发布包只含 `lib` + `dist` + `cordis.patch.yml`（构建产物已随包提交，**装完无需本地构建**）。
>
...
- **模糊、泛光、光轴、局部对比度现在支持了**：这些多趟渲染的特效已在本地实时执行（最长的泛光链要跑 16 趟）。若某张壁纸上看不到它们，多半是这些效果挂在**文字**或**合成层**对象上——那类对象本身还没有渲染，与特效引擎无关。
