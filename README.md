...
- **粒子的自转已实现**（2026-09-22）：花瓣、雪花的翻滚/倾斜现在按壁纸自身的 `rotationrandom` / `angularvelocityrandom` 渲染。**但能不能看出来取决于壁纸**：只有粒子规格里给了**非零**的旋转/角速度范围才会动 —— 本机 29 张里实测 **11 张**（15/121 个粒子对象）满足，其余多为该字段没有 `min`/`max` 或全零。另**已知缺口**：有 4 处把 `min`/`max` 写成**裸数字**（如 `-0.4`）的壁纸，因其解析层只认字符串/数组，参数会被静默丢弃 → 这几处自转不生效（见 `AGENT.md` §7.15）。
