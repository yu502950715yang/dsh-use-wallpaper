...
export const WallpaperSettingsSchema = z.object({
  ...
  // 应用级 Glow（整帧亮部发光）：阈值/强度默认值与 client 的 DEFAULTS 一致（2026-09-21 网格标定档）。
  glowEnabled: z.boolean().default(true),
  glowThreshold: z.number().min(0).max(0.99).default(0.75),
  glowStrength: z.number().min(0).max(4).default(0.4),
