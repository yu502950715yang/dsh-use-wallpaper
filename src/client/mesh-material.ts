...
export function parseMeshMaterial(jsonText: string): MeshMaterialSpec | null {
  ...
  const passes = (json as { passes?: unknown } | null)?.passes;
...
const FALLBACK_SPEC: MeshMaterialSpec = {
...
export function createMeshMaterial(spec: MeshMaterialSpec | null, texture: THREE.Texture | null): THREE.Material {
  const s: MeshMaterialSpec = spec ?? FALLBACK_SPEC;
...
function sharedWhiteTexture(): THREE.DataTexture {
  if (!whiteTex) {
