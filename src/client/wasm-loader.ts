// wasm 引擎加载胶水：只负责加载 wasm-pack --target web 产物并暴露模块类型。
// 主路径只用它的 `CpuParticleSim`（纯 CPU 粒子模拟，不需要 WebGPU）；旧的 WebGPU 渲染器已移除。
// wasm-pack --target web 产物（wasm/pkg/we_scene_wasm.js）：默认导出是实例化函数 __wbg_init。
