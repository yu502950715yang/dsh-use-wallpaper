...
// 「声明」= pass.samplerNames（**全部** sampler 声明）∪ samplerModes 的键：不能只看 samplerModes，
// 它只收带 mode 注解的槽。2937346640 的 godrays_combine 三个 sampler 全 "hidden"（无 mode），
// 只看它槽数算成 0 ⇒ g_Texture1 不进 material.uniforms ⇒ three 的 uniformsList（只在换 program 时
// 重算，已被 1×1 探针渲染冻住）不含该槽 ⇒ 之后 bindSlot 补上也不上传、sampler 停在默认 unit 0
// （读到 g_Texture0 的光线 RT）⇒ 整屏近黑、alpha 2×。**预建是上传该槽的唯一机会。**
export function effectSlotCount(pass: CompiledEffectPass): number {
  let count = pass.textureSlots.length;
  // `?? []` / `?? {}`：容忍早于这些字段构造的 pass 对象（如既有测试 fixture / 外部调用方），
  // 语义等同「无声明」⇒ 槽数退回 textures.length（既有行为）。
...
export class EffectRunner {
  ...
  private plannedPasses(): PlannedPass[] {
    ...
    try {
        ...
        for (let j = 0; j < slots; j++) {
          ...
          if (path && !boundByBind && this.isForeignRuntimeRt(path, pp.chainIndex)) {
            ...
            this.warnOnce(
              ...
              `[wallpaper-engine] 纹理槽引用了本链未声明的运行时 RT，跳过绑定（槽留空）: `
      ...
      for (const pp of planned) {
        ...
        for (const name of pp.unresolvedBinds ?? []) {
          if (!name) continue; // 空名 bind 无语义：不刷噪声
          this.warnOnce(
            `unresolved-bind:${this.id}:${pp.chainIndex}:${pp.passIndex}:${name}`,
            `[wallpaper-engine] bind 引用的名字不在本链内，该槽保持默认: ${name}`
        ...
        for (const b of pp.bindings) {
          ...
          if (tex) {
            ...
            // `previous` 不在 target 序列内：本槽保持默认。lwe 会回落到「当前内容提供者」，我方只有
            // slot 0 等价、slot ≥1 会拿到 textures[j] ⇒ 差异在此告警可诊断，但不猜成 readTex。
            this.warnOnce(
              `unresolved-previous:${this.id}:${key}:${b.slot}`,
              `[wallpaper-engine] bind 的 previous 不在 target 序列内，该槽回落到该槽默认: `
              + `g_Texture${b.slot}（壁纸 ${this.id}，pass ${key}）`,
            );
          }
        }
