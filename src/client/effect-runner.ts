...
export class EffectRunner {
  ...
  private async resolveTextureSlot(path: string | null): Promise<THREE.Texture | null> {
    ...
    // ⚠️ 效果纹理槽必须用 `alphaPriority: false`：效果 shader 把 R8/RG88 的通道当**遮罩数值**读
    // （pulse.frag 的 `.r`、shake.frag 的 `.rg`），而粒子语义会把 R8 变成 rgb 恒白、RG88 变成
    // (r,r,r,g) ⇒ 遮罩失效 ⇒ 本该局部的效果覆盖全图（真机 GTR 整屏脉冲 / 整屏抖动的根因）。
