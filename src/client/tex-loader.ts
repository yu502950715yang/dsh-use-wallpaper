...
async function videoTextureFromMp4(
  ...
  info: TexInfo,
  topDown: boolean,
  decorate: (tex: THREE.Texture) => THREE.Texture,
...
export async function textureFromTex(info: TexInfo, opts?: TexLoadOptions): Promise<THREE.Texture | null> {
  ...
  const withSprite = (tex: THREE.Texture): THREE.Texture => {
    const ud: Record<string, unknown> = { ...(tex.userData ?? {}) };
...
export function convertUnormToRgba(data: Uint8Array, format: number, alphaPriority = true): Uint8Array<ArrayBuffer> {
  if (format === TEX_FORMAT.RG88) {
    ...
    for (let i = 0, o = 0; i < data.length; i += 2, o += 4) {
      ...
      if (alphaPriority) {
        ...
        out[o] = r; out[o + 1] = g; out[o + 2] = 0; out[o + 3] = 255;
      }
    }
    return out;
  }
