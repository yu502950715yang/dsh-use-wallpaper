...
//
// 链路：WE 方言 → （规则①-⑨ 预处理为桌面 GLSL）→ @webgpu/glslang（GLSL→SPIR-V，entry_point
//   `main`）→ wasm 侧 spirv-webgpu-transform + naga spv-in → WGSL。JS 侧只产 SPIR-V（transform/spv-in
//   在 wasm 编译期做，一次非每帧；见 brief 裁决）。
//
...
// `await glslangNS.default()` 返回带 `compileGLSL` 的 Glslang 实例。NodeNext 下 CJS 默认导入被
// 类型解析为模块命名空间（`typeof import(...)`），故用显式类型桥接声明该可调用形态。
...
function loadGlslang(): Promise<GlslangApi> {
  // 浏览器 bundle：web-devel 工厂的 locateFile() 经 build-client.mjs 的 glslang-web-patch 插件
  // 改为读 globalThis.__DSH_GLSLANG_BASE__（DSH 插件静态路由前缀），以 fetch
  // /wallpapers/static/glslang.wasm。Node 测试（node-devel）用 fs 读 wasm，不受影响。
...
// "SPIR-V requires location for user input/output"；内置 gl_Position/gl_FragCoord 不声明）。顶点 out 与
// 片元 in 各按声明顺序从 0 编号，且有效果 shader 中 varyings 的 vert/frag 顺序一致 → 同名 varying 得到
// 相同 location（vertex 输出 ↔ fragment 输入匹配）。已带 layout(location=...) 的声明（如 o_Color）跳过；
// 数组 varying/attribute（`varying vec2 v_TexCoord[4];`，blur/downsample 多用）名字后有 `[N]`。
...
function assignInterfaceLocations(src: string, stage: Stage): string {
  ...
  for (const line of src.split('\n')) {
    ...
    if (m && !hasLayout) {
      ...
      // 后续接口须在上一个数组占用的槽之后，否则 location 重叠 → glslang 报错。
...
export function std140TypeInfo(typeStr: string): Std140TypeInfo | null {
  ...
  // size>16，mat4[2] 应为 2*64=128B 而非 2*16=32B）。
...
// 打包在 wasm `pack_std140_block`（JS 侧删除了重复的 std140WritePlan/packStd140* 副本，避免与 Rust
// 维护两份而再次引入 mat[N] stride bug，见 git log）。
...
// glslang 的 C 预处理器在编译时也会裁剪；但 JS 侧 `assignInterfaceLocations` 需在**裁剪后**的文本上
// 给 varying 编号（否则互斥多分支里每个分支声明都被编号，把选中分支 location 推到超过 WebGPU
// `maxInterStageShaderVariables`(=16)，如 blur_gaussian 的 `#if KERNEL==0/1/2`）。必须先于 varying
// 改写/编号执行。保持嵌套，未激活块内的非预处理行剔除。
// 关键：必须**模拟预处理宏状态**（而非只用初始注入宏集）——header guard（`#ifndef WE_COMMON_H` +
// `#define WE_COMMON_H`）防重复 include；若 `#define` 不累积进 `isDefined`，第二次 `#ifndef` 会误判
// 为真，把 common.h 重复内联（如 `frac(...)` 重定义报 "function already has a body"）。故遍历时
// `#define` 加入已定义集、`#undef` 移除。`valueDefines`（combos 值）供 `#if IDENT == N` 求值；
// `initDefined` 为一开始就视为已定义的宏（combos 名 + [COMBO] 默认宏），与"仅出现在 #if 表达式里被
// 兜底成 0 的标识符"区分——后者未定义（C 语义当 0），`#ifdef` 应判 false。
...
// `TypeFlags::IO_SHAREABLE`（见 effect.rs spv_to_wgsl 报 `NotIOShareableType`）。
//
// **左值写保底**：展开的前提是能改写所有下标引用。变量下标 `name[idx]` 作为**右值读**可改写为
// 三元选择链（`(idx==0?name_0:...)` 合法右值）；但作为**赋值左值**（`name[idx] = x`、`name[idx] += x`、
// `name[idx]++`）三元链是**非左值**，glslang 拒绝。因此若某数组存在**任意一处变量下标左值写**，
// 则**整个数组保留原样**（声明与所有下标引用都不展开/改写），交给调用方 per-pass 容错兜底
// （该 pass 跳过，不崩不白屏）。常量下标写（blur/gaussian 的 v_TexCoord[0]=...）与变量下标右值读
// 仍可正常展开。
...
function convertStage(
  ...
  // ④' WE 方言严格模式修正（复用 three 路径 shader-preprocessor 的成熟三连）：真实 WE 效果 shader
  // 在 glslang(Vulkan/desktop GLSL 4.50) 报
  ...
  // ④''' 按组合宏裁剪互斥 `#if/#elif/#else/#endif` 分支：glslang 的 C 预处理器在编译时也会裁剪，但
  //   JS 侧 `assignInterfaceLocations` 需在**裁剪后**文本上编号 varying（否则互斥多分支里每个分支的
  //   声明都被编号，把选中分支 location 推到超过 WebGPU `maxInterStageShaderVariables`(=16)，如
  //   blur/localcontrast_gaussian 的 `#if KERNEL==0/1/2`，wgpu 校验拒绝）。必须先行裁剪。
  //   求值用与 `buildDefines` 同一宏集（combos 优先 → [COMBO] 默认 → 未定义兜底 0），与 glslang
  //   语义一致；`#ifdef`/`#ifndef` 按"是否定义"处理（初始已定义集 = combos 名 + [COMBO] 默认宏，
  //   不含被兜底成 0 的 #if 标识符——后者未定义，`#ifdef` 应判 false）。
  s = expandIfBranches(
    s,
  ...
  // ⑤'' 数组 IO 展开：naga 24 的 Validator 校验 `NotIOShareableType`——`TypeFlags::IO_SHAREABLE`
  //   对数组顶层 IO 不成立，由 naga spv-in 产生的数组 varying/attribute（如 blur/localcontrast 的
  //   `varying vec2 v_TexCoord[4]` 采样偏移数组）spv-in 后必过不了校验，导致效果链 pass 编译失败、
  //   整链回退。WE 效果 shader 的数组下标恒为常量（0..N-1），故展开为 N 个**同名后缀**的单独声明
  //   （语义等价位点连续 location），并把常量下标引用改写为对应后缀名，使 SPIR-V 无数组 IO，
  //   naga spv-in 正常通过。
...
// 声明对应输入** → `interStageLocationsMatch` 判定不匹配 → 该 pass 被跳过。
// 故先把**两个 stage** 的 [COMBO] 默认合并进 pass.combos，再传给两个 stage 的转换。
function passCombos(pass: CompiledEffectPass): Record<string, number> {
  const merged: Record<string, number> = { ...pass.combos };
...
// WebGPU inter-stage 匹配校验：fragment 的每个 `layout(location=N)` 输入，必须存在相同 location
// 且相同类型的 vertex 输出。GL 对"未连接的 varying"提供默认值，WebGPU 严格校验
