...
| 2 | **`bind` 优先于 `textures`** | `CPass` 初始化段：`m_override.usertextures` 先填 `m_textures[index]`（`CPass.cpp:791-807`），`m_binds` 其后填同一 map（`CPass.cpp:809-818`，注释 *binds are set last as they're the most important to be set*）；两者都经 `TextureChainEntry` 链式回落，后写者成为链首 ⇒ bind 胜出。（**实现期订正 2026-09-15**：本行早先引的 `CImage.cpp:791-807` / `:809-819` 实为 `CImage::setupPasses` 的循环体、不是本规则的出处。） |
| 3 | **`name === "previous"` → `m_previousInput ?: (previous ?: expected)`** | `CPass::resolveTexture`（`CPass.cpp:86-109`）：先按 index 找 `m_fbos`，再按 index 查 `m_binds`；`"previous"` 返回 `m_previousInput`，缺失则回落到 `previous`（调用方传入的当前读端）。（**实现期注 2026-09-15**：本检出的 `CPass::m_fbos`（`CPass.h:182`）只读不写、恒空 —— 唯一读取点是 `CPass.cpp:90`，属死代码；具名 RT 实际只由 `m_binds` + `resolveFBO` 提供。） |
